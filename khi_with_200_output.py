...
#  Modified to save PNG output files
...
def myPlot(time, output_count):
    ...
    # Save figure to file
    filename = os.path.join(output_dir, f"khi_frame_{output_count:04d}.png")
    plt.savefig(filename, dpi=100, bbox_inches='tight')
    print(f"Saved frame {output_count} at time {time:.3f} to {filename}")
...
print(f"Initial state saved to {latest_file}")
outputCount = 1
...
print(f"\nSimulation complete. {outputCount} frames saved to {output_dir}/")
